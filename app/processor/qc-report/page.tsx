...
function ProcessorQcReportContent() {
  ...
  function onCertChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setCertFile(f);
    setCertName(f.name);
    setErrors((s) => ({ ...s, cert: undefined })); // Perbaiki: set ke undefined jika valid
  }
  ...
  function validateForm() {
    ...
    if (!certFile)
      next.cert = "Certificate of healthy shrimp harus diunggah (PDF/JPG/PNG)";
    setErrors(next);
    ...
    <div className="p-6 max-w-md mx-auto min-h-screen flex flex-col items-center justify-center text-black">
      ...
      <Card className="mt-4 bg-white/10 border-white/20 backdrop-blur-xl w-full">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <label className="block text-sm mb-1 text-black">
              Certificate of Healthy Shrimp (PDF / JPG / PNG)
            </label>
            <input
              type="file"
            ...
            />
            {errors.cert && (
              <div className="text-sm text-destructive mt-1">{errors.cert}</div>
            )}
            {certName && (
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded px-3 py-2 bg-white/10 text-black"
            >
          ...
          {submitMessage && (
            <div className="text-sm mt-2 text-center">{submitMessage}</div>
          )}
        </CardContent>
      </Card>
    </div>
