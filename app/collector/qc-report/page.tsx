...
export default function QcReportPage() {
    ...
    async function patchQc(metadata: any) {
      if (!batchId || !userId || !role) return;

      const body = {
        catch_time: toISO(receivedAt),
        metadata,
      };

      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/batches/${batchId}`,
        {
          method: "PATCH",
      ...
      const newList = saved.map((b: any) =>
        b.id === batchId ? { ...b, qc: metadata } : b
      );
