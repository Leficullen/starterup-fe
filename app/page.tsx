...
export default function Home() {
  ...
  const Solutions = [
    "Real-time contamination monitoring at every stage",
    "Integrated traceability from farm to export",
    "Automated alert system for early detection",
    "Accurate identification of pollution sources",
    "Digital certification for international trust",
  ];
  const timelineItems = [
    {
      role: "Farmer",
            ...
            <div>
              <ul className="list-disc mt-3 md:mt-0 md:ml-16 mx-[15px] text-red-800">
                {Problems.map((problem) => (
                  <li className="text-sm md:text-lg">{problem}</li>
                ))}
              </ul>
            </div>
          </div>
